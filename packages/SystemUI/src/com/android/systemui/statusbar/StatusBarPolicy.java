...
public class StatusBarPolicy {
	...
	private static final int[][] sSignalImages = {
			{ R.drawable.stat_sys_signal_0, R.drawable.stat_sys_signal_1,
					R.drawable.stat_sys_signal_2, R.drawable.stat_sys_signal_3,
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (action.equals(Intent.ACTION_BATTERY_CHANGED)) {
				...
				updateGps(intent);
					...
					|| action
							.equals(AudioManager.VIBRATE_SETTING_CHANGED_ACTION)) {
				updateVolume();
				...
				updateConnectivity(intent);
					...
					.equals(WimaxManagerConstants.WIMAX_ENABLED_STATUS_CHANGED)
					|| action
							.equals(WimaxManagerConstants.SIGNAL_LEVEL_CHANGED_ACTION)
					|| action
	...
	public StatusBarPolicy(Context context) {
		...
		mStorageManager
				.registerListener(new com.android.systemui.usb.StorageNotification(
						context));

		// battery
		mService.setIcon("battery",
				com.android.internal.R.drawable.stat_sys_battery_unknown, 0);
		...
		// data_connection
		mService.setIcon("data_connection",
				R.drawable.stat_sys_data_connected_g, 0);
		...
		// Sync state
		mService.setIcon("sync_active",
				com.android.internal.R.drawable.stat_notify_sync_anim0, 0);
		mService.setIcon("sync_failing",
				com.android.internal.R.drawable.stat_notify_sync_error, 0);
	...
	private final void updateBattery(Intent intent) {
		...
		if (false) {
			Slog.d(TAG, "updateBattery level=" + level + " plugged=" + plugged
					+ " mBatteryPlugged=" + mBatteryPlugged + " mBatteryLevel="
					+ mBatteryLevel + " mBatteryFirst=" + mBatteryFirst);
		}
		 ...
		 */
		if (false) {
			Slog.d(TAG, "plugged=" + plugged + " oldPlugged=" + oldPlugged
					+ " level=" + level);
		}
	...
	private void setBatteryLevel(View parent, int id, int height,
			int background, int level) {
	...
	private void showLowBatteryWarning() {
		...
		if (mBatteryLevelTextView != null) {
			...
			mBatteryLevelTextView = (TextView) v
					.findViewById(R.id.level_percent);
	...
	private void updateConnectivity(Intent intent) {
		...
		int connectionStatus = intent.getIntExtra(
				ConnectivityManager.EXTRA_INET_CONDITION, 0);

		int inetCondition = (connectionStatus > INET_CONDITION_THRESHOLD ? 1
				: 0);
	...
	private final void updateSignalStrength() {
		...
		if (!isCdma()) {
			...
			if ((mPhoneState == TelephonyManager.CALL_STATE_IDLE) && isEvdo()) {
				...
				if (false) {
					Slog.d(TAG, "use Evdo level=" + iconLevel
	...
	private final void updateDataIcon() {
		...
		if (!isCdma()) {
			...
			if (mSimState == IccCard.State.READY
					|| mSimState == IccCard.State.UNKNOWN) {
				if (hasService()
		...
		try {
			mBatteryStats.notePhoneDataConnectionState(mPhone.getNetworkType(),
					visible);
	...
	private final void updateVolume() {
		...
		final int iconId = audioManager
				.shouldVibrate(AudioManager.VIBRATE_TYPE_RINGER) ? R.drawable.stat_sys_ringer_vibrate
	...
	private final void updateWiMAX(Intent intent) {
		...
		if (action.equals(WimaxManagerConstants.WIMAX_ENABLED_STATUS_CHANGED)) {
			int mWimaxStatus = intent.getIntExtra(
					WimaxManagerConstants.EXTRA_WIMAX_STATUS,
			...
			case WimaxManagerConstants.WIMAX_STATUS_DISABLED:
				...
				.equals(WimaxManagerConstants.SIGNAL_LEVEL_CHANGED_ACTION)) {
			mWimaxSignal = intent.getIntExtra(
					WimaxManagerConstants.EXTRA_NEW_SIGNAL_LEVEL, 0);
		} else if (action
				.equals(WimaxManagerConstants.WIMAX_STATE_CHANGED_ACTION)) {
	...
	private final void updateGps(Intent intent) {
		final String action = intent.getAction();
		final boolean enabled = intent.getBooleanExtra(
				LocationManager.EXTRA_GPS_ENABLED, false);

		if (action.equals(LocationManager.GPS_FIX_CHANGE_ACTION) && enabled) {
			// GPS is getting fixes
			mService.setIcon("gps",
					com.android.internal.R.drawable.stat_sys_gps_on, 0);
	...
	private final void updateTTY(Intent intent) {
		final String action = intent.getAction();
		final boolean enabled = intent.getBooleanExtra(TtyIntent.TTY_ENABLED,
				false);
	...
	private final void updateCdmaRoamingIcon(ServiceState state) {
		...
		if (iconIndex == -1) {
			Slog
					.e(TAG,
							"getCdmaEriIconIndex returned null, skipping ERI icon update");
			return;
		}
		...
		if (iconMode == -1) {
			Slog
					.e(TAG,
							"getCdmeEriIconMode returned null, skipping ERI icon update");
			return;
		}
		...
		case EriInfo.ROAMING_ICON_MODE_NORMAL:
			if (iconIndex >= iconList.length) {
				Slog.e(TAG, "unknown iconIndex " + iconIndex
						+ ", skipping ERI icon update");
				return;
			}
		...
		case EriInfo.ROAMING_ICON_MODE_FLASH:
			mService.setIcon("cdma_eri",
					R.drawable.stat_sys_roaming_cdma_flash, 0);
			mService.setIconVisibility("cdma_eri", true);
			break;
